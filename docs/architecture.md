...
openfang-desktop        Tauri 2.0 desktop app (WebView + system tray)
    |
openfang-api            REST/WS/SSE API server (Axum 0.8), 177 endpoints
    |
...
| **openfang-cli** | Clap-based CLI plus ratatui TUI. Supports all commands: `init`, `start`, `status`, `doctor`, `agent spawn/list/chat/kill`, `workflow list/create/run`, `builder analyze/submit/jobs/job`, `approvals list/approve/reject`, `trigger list/create/delete`, `migrate`, `skill install/list/remove/search/create`, `channel list/setup/test/enable/disable`, `config show/edit`, `chat`, `mcp`. Daemon auto-detect: checks `~/.openfang/daemon.json` and health pings; uses HTTP when a daemon is running, boots an in-process kernel as fallback where supported. Built-in MCP server mode. |
...
5. Initialize model catalog
   - Build ModelCatalog with 200 builtin models, 23 aliases, 42 providers
...
5. **Module dispatch**: Based on `manifest.module`:
   - `builtin:router`: native deterministic goal router
...
Three native driver implementations cover all 42 providers with 200 models:
...
- **200 builtin models** across 30+ model families (Claude, GPT, Gemini, DeepSeek, Llama, Mixtral, Command, Jamba, Grok, Qwen, MiniMax, Zhipu, Moonshot, etc.)
- **23 aliases** for convenience (e.g., `claude` -> `claude-sonnet-4-20250514`, `grok` -> `grok-2`)
- **42 providers** with authentication status detection
...
|  | (200 models)    |  | (cost tracking)  |  | (auto-select)     |   |
|  +----------------+  +------------------+  +-------------------+   |
|  +----------------+  +------------------+  +-------------------+   |
